...
void MyMainFrame::HandleMenu()
{
   ...
   switch (id) 
   {
      ...
      case M_FILE_OPEN:
      {
         ...
         printf("fIniDir = %s\n", fi.fIniDir);
         new TGFileDialog(gClient->GetRoot(), nullptr, kFDOpen, &fi);
...
void MyMainFrame::DoDraw()
{
   ...
   //t1->SetBranchAddress(Form("channel_%i",ChNum), &channel_info);
...
MyMainFrame::MyMainFrame(const TGWindow *p, UInt_t w, UInt_t h, TString s):TGMainFrame(p, w, h)
{
   ...
   myc = new TCanvas("MyCanvas", 10,10,wid);
   ...
   fMenuFile->Connect("Activated(Int_t)", "MyMainFrame", this,
                      "HandleMenu(Int_t)");
   fMenuFile->Connect("PoppedUp()", "MyMainFrame", this, "");
   fMenuFile->Connect("PoppedDown()", "MyMainFrame", this, "");
